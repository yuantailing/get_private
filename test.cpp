...
class A {
    ...
    auto foo(T a, U b) const -> decltype(a + b) { return a + b; }
    static double y;
...
DEFINE_GET_PRIVATE(double (A::*)(int, double) const, get_A_foo, &A::foo)
DEFINE_GET_PRIVATE(std::string (A::*)(std::string, std::string) const, get_A_foo2, &A::foo)
...
int main() {
    ...
    a.*get_A_x() = 123;
    get_A_y() = 4.56;
    std::cout << (a.*get_A_foo())(1, 2.34) << std::endl;
    std::cout << (a.*get_A_foo2())("Hello", "World") << std::endl;
    std::cout << (*get_A_bar())(5, 6) << std::endl;
    std::cout << (*get_A_bar2())(7.89) << std::endl;
    a.check();
}
